...
#decap_file_directory = os.path.dirname(os.path.realpath(__file__))
decap_file_directory = os.getenv("DECAP_DIRECTORY")

DECAP_DECODER_CONFIG_PATH = os.path.join(decap_file_directory, "decoder_config.pkl")#os.path.join(os.getenv("DATA_DIRECTORY_ROOT"), "decap/decoder_config.pkl")
...
def decoding_batched(model, clip_features, compute_scores : bool = False):
    """
    ...
    try:
        ...
        for tokens_elem in tokens:
            ...
            output = _Tokenizer.decode(output_list)
            outputs.append(output.split('<|endoftext|>')[0] + '<|endoftext|>')
    except:
        outputs = None
