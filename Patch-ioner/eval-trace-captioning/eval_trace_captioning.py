...
def evaluate(model_name, 
    ...
    if caption_from != 'patches':
        ...
        caption_from_str = ""
    evaluation_dataset_name = 'flickr30k-recap' if 'flickr30k' in evaluation_dataset else 'coco-recap'
    attn_weight_str = '' if not use_attention_weighting else "_attn-weighted"
    output_file = f"./annotations/predictions_{model_name}-{evaluation_dataset_name}-{crop_str}{gaussian_str}{caption_from_str}{attn_weight_str}.json"
    ...
    print(f"Use Attention Weighting: {use_attention_weighting}")
    ...
    base_path = '/raid/datasets/flickr30k-images' if 'flickr30k' in evaluation_dataset else'/raid/datasets/coco/val2017'
    backup_path = '/raid/datasets/coco/train2017'

    coco_base_path = '/raid/datasets/coco/'
    ...
    for i, (img_id, img_obj) in enumerate(test_set.items()):
        if 'coco' in evaluation_dataset:
            img_id = img_id.zfill(12) 
        img_path = os.path.join(base_path, f"{img_id}.jpg")
        if not os.path.exists(img_path):
            img_path = os.path.join(backup_path, f"{img_id}.jpg")
        
        for capt, trace in zip(img_obj['captions'], img_obj['traces']):
            samples['gt_captions'].append(capt)
            samples['img_paths'].append(img_path)
            samples['traces'].append(trace)
    
    preds = []
    n_imgs = len(samples['img_paths'])
    ...
    for i in tqdm(range(n_batch)):
        ...
        if limit is not None and start > limit:
            print(f"Limit reached at batch {i}, stopping early.")
            break
        ...
        traces = samples['traces'][start:end]
        for im_idx, im_path in enumerate(img_paths):
            # check if file is accessible for permissions
            if not os.access(im_path, os.R_OK):
                # get the file name and try to access it from the backup path
                im_name = os.path.basename(im_path)
                # we need also the child folder of the path
                im_folder = os.path.basename(os.path.dirname(im_path))
                # get the new path
                im_path = os.path.join(coco_base_path, im_folder, im_name)
                # update the path
                img_paths[im_idx] = im_path
        ...
        if measure_flops and FLOP_COUNTER_AVAILABLE and i == 0:
            ...
            model_kwargs = {
                ...
                'use_attention_tracing': use_attention_weighting
            }
        ...
        curr_inf_preds = outs['cls_capt'] if caption_from == 'cls' else outs['trace_capts']
        if isinstance(curr_inf_preds, list):
            print(f"Batch {i+1}/{n_batch}: {len(curr_inf_preds)} predictions | curr_inf_preds content: ", curr_inf_preds[:5])
            preds.extend(curr_inf_preds)
        else:
            assert isinstance(curr_inf_preds, str), f"Expected predictions to be a list or string, got {type(curr_inf_preds)}"
            preds.append(curr_inf_preds)
    ...
    result_data = {
        ...
        "use_attn_map_for_bboxes": use_attn_map_for_bboxes,
        "use_attention_weighting": use_attention_weighting,
        # Timing measurements
    ...
    result_data_enhanced = {**result_data, "computation_details": computation_details, "other_config_data": other_config_data}
    # Save additional timing and FLOP data
    enhanced_save_path = output_file.replace('.json', '_enhanced.pkl')
    with open(enhanced_save_path, 'wb') as f:
        pickle.dump(result_data_enhanced, f)
    print(f"Saved enhanced results (including timing data) to {enhanced_save_path}")
...
def main():
    ...
    parser.add_argument("--caption_bboxes_type", type=str, default=None, help="Type of bounding boxes for captions.")
    parser.add_argument("--double_dino_last_layer", action="store_true", help="Whether to double the DINO last layer.")
    parser.add_argument("--use_attention_weighting", action="store_true", help="Whether to weight patches with the attention map.")
    ...
    parser.add_argument("--use_attn_map_for_bboxes", action='store_true', default=False, help="Whether to use the attention map for the bounding boxes")
    parser.add_argument("--measure_flops", action='store_true', default=False, help="Whether to measure FLOPs during evaluation")
    ...
    parser.add_argument("--limit", type=int, default=None, help="Limit the number of images to process. Default is None (process all images).")
    ...
    output_file_path = evaluate(
        ...
        use_attention_weighting=args.use_attention_weighting,
