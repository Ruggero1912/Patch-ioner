...
if __name__ == "__main__":
    ...
    save_path = os.path.join(args.memory_base_path, f"memory/{args.memory_id}")
    
    print(F"Will use '{args.memory_id = } and the input text corpus is at {input_text_corpus_path = }'")
    print(F"Will save the outputs in '{save_path = }'")
    ...
    print(f"Going to use '{args.vl_model}' as CLIP model")
    ...
    batch_size = 128
    for idx in tqdm(range(0,len(textual_data),batch_size), dynamic_ncols=True):
